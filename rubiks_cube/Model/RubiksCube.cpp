...
char RubiksCube::getColorLetter(COLOR color){
    ...
    //performing random shuffle from initial solved state of the cube

    vector<RubiksCube::MOVE> RubiksCube::randomShuffleCube(unsigned int times){
        vector<MOVE> moves_performed;

        for(unsigned int i=0;i<times;i++){
            unsigned int selectMove=(rand()%18);    //selecting one of the possible moves among all 18 moves applicable the the cube
    ...
    //helper function for returning strings of the corner

    string RubiksCube::getCornerColorString(uint_8 ind) const{
